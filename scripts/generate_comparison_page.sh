...
<head>
  ...
  <style>
    ...
    .action-btn { padding: 5px 10px; border-radius: 3px; cursor: pointer; }
    .delete-btn { background-color: #f44336; color: white; border: none; margin-left: 5px; }
    ...
    .modal-buttons button { margin-left: 10px; padding: 8px 16px; border-radius: 4px; cursor: pointer; }
    .btn-cancel { background-color: #f2f2f2; border: 1px solid #ddd; }
...
<body>
  ...
  <script>
    ...
    function populateTable() {
      ...
      filteredTests.forEach(test => {
        ...
        row.innerHTML = `
          ...
          <td>${test.samples}</td>
          <td>
            <button class="action-btn delete-btn" data-id="${test.id}" onclick="showDeleteConfirmation('${test.id}', '${formatDate(test.date)}')">Delete</button>
          </td>
        `;
    ...
    function showDeleteConfirmation(id, formattedDate) {
      testToDelete = id;
      document.getElementById('deleteTestDate').textContent = `Test from: ${formattedDate}`;
    ...
    function deleteTest(id) {
      ...
      .then(response => {
        if (response.ok) {
          ...
          // If API server fails, try the PHP endpoint as fallback
          return fetch(`delete-test.php?id=${id}`, {
            method: 'POST'
          })
          .then(resp => {
            if (resp.ok) {
              ...
              throw new Error('Failed to delete test with both methods');
            }
          });
        }
      })
      .then(data => {
        // Remove the test from our arrays
        allTests = allTests.filter(test => test.id !== id);
        filteredTests = filteredTests.filter(test => test.id !== id);
        ...
        // Also store the deletion in localStorage to remember it across sessions
        const deletedTests = JSON.parse(localStorage.getItem('deletedTests') || '[]');
        deletedTests.push(id);
        localStorage.setItem('deletedTests', JSON.stringify(deletedTests));
      })
      .catch(error => {
        console.error('Error deleting test:', error);
        
        // If both server methods fail, offer manual deletion from UI only
        if (confirm('Server deletion failed. Remove this test from the UI only? (Note: The file will still exist on the server)')) {
          // Remove from UI only
          allTests = allTests.filter(test => test.id !== id);
          filteredTests = filteredTests.filter(test => test.id !== id);
    ...
    // Filter out tests that were deleted in previous sessions
    function filterDeletedTests() {
      const deletedTests = JSON.parse(localStorage.getItem('deletedTests') || '[]');
      if (deletedTests.length > 0) {
        allTests = allTests.filter(test => !deletedTests.includes(test.id));
        filteredTests = filteredTests.filter(test => !deletedTests.includes(test.id));
      }
    }
    ...
    window.addEventListener('DOMContentLoaded', () => {
      initComparison()
        .then(() => {
          // Filter out deleted tests after loading
          filterDeletedTests();
          updateCharts();
          populateTable();
          populateTestSelectors();
